...
uint32_t IndexMetadata::GetSerializedSize() const {
  return 4                              // MAGIC_NUM
       + 4                              // index_id
       + 4 + index_name_.length()       // index_name (len prefix + data)
       + 4                              // table_id
       + 4                              // key_map_ size
       + key_map_.size() * 4;           // each key_map entry
}

uint32_t IndexMetadata::DeserializeFrom(char *buf, IndexMetadata *&index_meta) {
  if (index_meta != nullptr) {
    LOG(WARNING) << "Pointer object index info is not null in index info deserialize." << std::endl;
  }
  char *p = buf;
...
Index *IndexInfo::CreateIndex(BufferPoolManager *buffer_pool_manager, const string &index_type) {
  ...
  if (index_type == "bptree") {
    ...
    else { LOG(ERROR) << "GenericKey size is too large"; return nullptr; }
  } else {
    return nullptr;
  }
